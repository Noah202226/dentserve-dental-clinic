...
export default function InstallmentsModal({ transaction, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-[99999] bg-black/60 backdrop-blur-sm flex justify-center items-center">
      <div className="bg-base-100 dark:bg-gray-900 w-full max-w-lg rounded-2xl border border-gray-700 shadow-2xl overflow-hidden">
        {/* Header */}
        ...
        {/* Summary */}
        <div className="grid grid-cols-3 text-center border-b border-gray-700 p-3 bg-base-200 dark:bg-gray-800">
          <div>
          ...
          {loading ? (
            ...
            <ul className="space-y-3">
              {installments.map((i) => (
                <li
                  key={i.$id}
                  className="bg-base-200 dark:bg-gray-800 p-4 rounded-lg border border-gray-700 hover:border-primary transition"
                >
                  ...
                  {i.note && (
                    <p className="text-xs text-gray-500 mt-1">{i.note}</p>
                  )}
                </li>
              ))}
            </ul>
